...
func main() {
	...
	*/
	...
	fmt.Println(search([]int{3, 4, 5, 6, 1, 2}, 2))
}

func search(nums []int, target int) int {
	peak, n := findPeakElement(nums), len(nums)
	if n == 1 {
		if nums[0] == target {
			return 0
		}
	...
	if peak == n-1 && nums[0] > nums[peak] {
		if target >= nums[n>>1] && target <= nums[n-1] {
			return searchDevide(nums, n>>1, n-1, target)
		} else {
			return search(nums[0:n>>1], target)
		}

	} else {
		return searchKnownChange(nums, peak, target)
	}
}
func searchKnownChange(nums []int, peak, target int) int {
...
func searchDevide(nums []int, l, r, target int) int {
	if l == r {
		if nums[l] == target {
			return l
		}
	...
	mid := (l + r) >> 1
	if target > nums[mid] {
		return searchDevide(nums, mid+1, r, target)
	}
	if target < nums[mid] {
		return searchDevide(nums, l, mid-1, target)
	}
